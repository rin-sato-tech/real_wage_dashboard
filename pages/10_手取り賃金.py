...
try:
    period_log_df = load_snapshot(
        "take_home_period_log_decomposition.csv"
    )

    burden_change_df = load_snapshot(
        "take_home_burden_change_summary.csv"
    )

    burden_shapley_df = load_snapshot(
...
with col1:
    st.metric(
        label="2025年 額面賃金",
        value=(
            f"{end['gross_salary_yen'] / 10_000:,.1f}"
            "万円"
        ),
        delta=(
            f"{gross_change:+.2f}% "
...
with col2:
    st.metric(
        label="2025年 名目手取り",
        value=(
            f"{end['nominal_take_home_yen'] / 10_000:,.1f}"
            "万円"
        ),
        delta=(
            f"{nominal_take_home_change:+.2f}% "
            "（1990年比）"
        ),
    )
...
with col3:
    st.metric(
        label="2025年 実質手取り",
        value=(
            f"{end['real_take_home_yen'] / 10_000:,.1f}"
            "万円"
        ),
        delta=(
            f"{real_take_home_change:+.2f}% "
            "（1990年比）"
        ),
    )
...
with col4:
    st.metric(
        label="2025年 実効負担率",
        value=(
            f"{end['effective_burden_rate'] * 100:.2f}%"
        ),
        delta=(
            f"{burden_change_pt:+.2f}pt "
            "（1990年比）"
        ),
        delta_color="inverse",
    )
...
st.altair_chart(
    create_period_decomposition_chart(
        period_log_df
    ),
    width="stretch",
)
...
st.header(
    "5. 実質手取りの4要因Shapley分解"
)

st.altair_chart(
    create_real_shapley_chart(
        real_shapley_df
    ),
    width="stretch",
)
...
long_term = real_shapley_df.loc[
    real_shapley_df["period"]
    == "1990→2025"
]
