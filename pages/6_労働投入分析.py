...
st.markdown(
    """
    月額賃金の変化を、1時間あたり賃金（概算）と労働時間に分解し、
    さらに労働時間の変化を所定内・所定外労働時間、出勤日数へ段階的に分解します。
    """
)
...
st.markdown(
    """
    **月額賃金が変化したのは、1時間あたりの賃金が変わったからか、それとも働く時間が変わったからか？**
...
st.caption(
    "月額賃金だけでは、賃金単価が上がったのか、働く時間が増減したのかを区別できません。"
    "そのため、まず賃金単価と労働時間に分け、さらに労働時間を段階的に分解します。"
)
...
st.markdown(
    ...
    **月額のきまって支給する給与は
    {wage_summary["wage_change_pct"]:+.2f}%増加**しました。

    一方、1時間あたり賃金（概算、年間加重）は
    {wage_summary["hourly_wage_change_pct"]:+.2f}%、
    総実労働時間は
    {wage_summary["total_hours_change_pct"]:+.2f}%となりました。

    月額賃金の上昇は**1時間あたり賃金（概算）の上昇**によって押し上げられた一方、
...
with st.expander("なぜ2015年と2025年を比較するのか"):
    st.markdown(
        """
        2015年と2025年は、どちらも1～12月のデータが揃った完全な年です。

        今回は、短期的な月次変動ではなく約10年間の構造変化を見るため、
        2015年平均と2025年平均を基準比較としました。

        また、この期間にはコロナ前・コロナ期・その後が含まれるため、
        長期変化と一時的な変動を区別して確認できます。

        ただし、2015年を起点とすることが唯一の選択ではないため、
        年次推移も併せて確認します。
        """
    )
...
with st.expander("雇用形態別の補助分析"):
    st.write(
        "就業形態計の平均値は、一般労働者とパートタイム労働者の構成比変化でも動きます。"
        "そのため、「出勤日数減少」が単なる構成変化によるものかを確認する目的で"
        "補助的に雇用形態別比較を行います。"
    )
    ...
    st.markdown(
        """
        一般労働者・パートタイム労働者の双方で、出勤日数と所定内労働時間が減少しています。

        一方、1出勤日当たり所定内労働時間は両雇用形態ともわずかに増加しています。

        したがって、就業形態計で確認された出勤日数減少は、雇用形態構成比の変化だけでは説明できません。
        """
    )
...
st.markdown(
    ...
    その伸びは「1時間あたり賃金（概算）」の伸びより小さくなっています。

    要因分解では、「1時間あたり賃金（概算）」の上昇が月額賃金を押し上げた一方、
