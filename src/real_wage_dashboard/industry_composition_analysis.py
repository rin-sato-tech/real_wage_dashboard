...
from real_wage_dashboard.industry_analysis import MAIN_INDUSTRIES
...
def create_industry_employment_monthly_dataframe(
    ...
    df["monthly_employment"] = (
        df["previous_month_end_employment"]
        + df["current_month_end_employment"]
    ) / 2
...
def create_industry_composition_decomposition(
    ...
    df["within_wage_effect"] = (
        df["start_share"]
        * df["wage_change"]
    )

    df["composition_effect"] = (
        df["start_wage"]
        * df["share_change"]
    )
    ...
    df["centered_composition_effect"] = (
        df["start_wage"]
        - start_average_wage
    ) * df["share_change"]
    ...
    df["wage_level_group"] = df[
        "start_wage"
    ].apply(
        lambda x: (
            "平均以上"
            if x >= start_average_wage
            else "平均未満"
        )
    )

    df["share_change_group"] = df[
        "share_change"
    ].apply(
        lambda x: (
            "シェア上昇"
