...
def main() -> None:
    ...
    main_series = calculate_take_home_time_series(
    ...
    main_series = add_real_take_home_metrics(
        take_home_df=main_series,
        annual_cpi_df=annual_cpi_df,
        base_year=START_YEAR,
    )

    main_series = add_deduction_component_rates(main_series)

    # ----------------------------------------
    # 5. 期間別対数分解
    # ----------------------------------------

    log_decomposition = create_take_home_period_log_decomposition(
        df=main_series,
        periods=PERIODS,
    )
    ...
    burden_change = create_deduction_burden_change_summary(
        df=main_series,
        periods=PERIODS,
    )
    ...
    burden_shapley = create_burden_three_factor_shapley_decomposition(
        annual_wage_df=annual_wage_df,
        rule_tables=rules,
        periods=PERIODS,
        sex="male",
    ...
    real_shapley = create_real_take_home_four_factor_shapley_decomposition(
        annual_wage_df=annual_wage_df,
        annual_cpi_df=annual_cpi_df,
        rule_tables=rules,
        periods=PERIODS,
    ...
    outputs = {
        "take_home_main_series.csv": main_series,
        "take_home_period_log_decomposition.csv": log_decomposition,
        "take_home_burden_change_summary.csv": burden_change,
        "take_home_burden_shapley_3factor.csv": burden_shapley,
        "take_home_real_shapley_4factor.csv": real_shapley,
    }
    ...
    print("\n=== 1990→2025 4要因Shapley ===")

    print(
        real_shapley.loc[real_shapley["period"] == "1990→2025"].to_string(
            index=False,
            float_format=lambda x: f"{x:,.6f}",
        )
    )
