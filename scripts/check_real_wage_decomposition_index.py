...
def validate_tl_sheet(df: pd.DataFrame, path: Path) -> None:
    ...
    for candidates in required_keyword_groups:
        if not any(keyword in header_text for keyword in candidates):
...
def summarize_chained_period_change(
    ...
    start_yearを基準年とし、start_year+1～end_yearの前年比を使用する。
    """
...
def build_cpi_sensitivity(
    ...
    sensitivity = sensitivity_cpi_df.rename(
        columns={
            "cpi": "all_items_cpi",
            "cpi_yoy_pct": "all_items_cpi_yoy_pct",
        }
    )
    ...
    result = (
        ...
        .merge(
            main[
                ["year", "main_cpi", "main_cpi_yoy_pct"]
            ],
            on="year",
            how="inner",
            validate="one_to_one",
        ...
        .merge(
            sensitivity[
                ["year", "all_items_cpi", "all_items_cpi_yoy_pct"]
            ],
            on="year",
            how="inner",
            validate="one_to_one",
    ...
    result["real_yoy_all_items_cpi_pct"] = (
        (
            (1 + nominal_rate)
            / (1 + result["all_items_cpi_yoy_pct"] / 100)
            - 1
        )
        * 100
    )
...
def validate_results(
    ...
    max_index_difference = (
        period_df["real_index_difference"]
        .abs()
        .max()
    )

    if max_index_difference > OFFICIAL_REAL_INDEX_TOLERANCE:
    ...
    decomposition_error = (
        period_df["nominal_log_contribution"]
        + period_df["price_log_contribution"]
        - period_df["real_log_change"]
    ).abs().max()
...
def print_index_validation(
    ...
    year_2024 = period.loc[period["year"] == 2024]

    if len(year_2024) == 1:
        row = year_2024.iloc[0]
...
def print_sensitivity(
    ...
    columns = [
        ...
        "published_nominal_yoy_pct",
        "main_cpi_yoy_pct",
        "all_items_cpi_yoy_pct",
        "real_yoy_main_cpi_pct",
        "real_yoy_all_items_cpi_pct",
...
def main() -> None:
    ...
    all_items_cpi_monthly_df = load_cpi_dataframe(
        app_id=app_id,
        series_code=CPI_SERIES[SENSITIVITY_CPI_SERIES],
    )
    all_items_cpi_annual_df = create_complete_annual_cpi(
        all_items_cpi_monthly_df
    )

    analysis_df = build_main_analysis(
    ...
    sensitivity_df = build_cpi_sensitivity(
        ...
        sensitivity_cpi_df=all_items_cpi_annual_df,
    )
